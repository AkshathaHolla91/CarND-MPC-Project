...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N - 1; t++) {
      fg[0] += 175	*CppAD::pow(vars[delta_start + t], 2);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  std::vector<double> solved_values={solution.x[delta_start], solution.x[a_start]};
  for(int i=0;i<N;i++){
    solved_values.push_back(solution.x[x_start+i]);
    solved_values.push_back(solution.x[y_start+i]);
  }
